...
def get_item_from_id(db: Session,item_id:int):
...
def edit_user_item(db: Session, item: schemas.ItemCreate, item_id: int):
    db_item = db.query(models.Item).filter(models.Item.id == item_id).first()
    db_item.date = item.date
    db_item.name = item.name
    db_item.status = item.status
    db_item.health_issues = item.health_issues
    db_item.want_description = item.want_description
