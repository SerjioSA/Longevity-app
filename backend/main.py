...
@app.put("/items/edit/{item_id}/", response_model=schemas.Item)
def edit_item_for_user(
    ...
    return crud.edit_user_item(db=db, item=new_item, item_id=item_id)
...
def read_item_from_id(item_id: int, db: Session = Depends(get_db)):
    item = crud.get_item_from_id(db, item_id=item_id)
    return item
